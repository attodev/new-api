...
autoDetect:
	...
	if host == "" {
		return nil, fmt.Errorf("cannot determine passkey origin, please specify in system settings or passkey settings. current host: '%s', server address: '%s'", r.Host, system_setting.ServerAddress)
	}
	if scheme == "" {
		scheme = "https"
	}
...
func resolveRPID(r *http.Request, settings *system_setting.PasskeySettings, origins []string) (string, error) {
	...
	if len(origins) == 0 {
		return "", errors.New("passkey origin not configured, cannot derive RPID")
	}
	parsed, err := url.Parse(origins[0])
	if err != nil {
		return "", fmt.Errorf("cannot parse passkey origin: %w", err)
	}
