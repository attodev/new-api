...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (err error) {
	...
	if relayInfo != nil && (relayInfo.BillingSource == BillingSourceSubscription || relayInfo.BillingSource == BillingSourceOrganizationSubscription) {
		...
		// Wallet
		err = AdjustWalletQuotaForUser(relayInfo.UserId, int64(quota))
		if err != nil {
			return err
		}
	}
...
func checkAndSendQuotaNotify(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int) {
	gopool.Go(func() {
		...
		quotaTooLow := false
		consumeQuota := int64(quota + preConsumedQuota)
		...
		if quotaTooLow {
			...
			// generate different content formats based on notification type
			var content string
			...
			if notifyType == dto.NotifyTypeBark {
				// Bark push uses short text, no HTML support
				...
				// default content format for Email and Webhook (HTML supported)
				content = "{{value}}, current remaining quota is {{value}}, please top up to avoid service interruption.<br/>Top-up link: <a href='{{value}}'>{{value}}</a>"
