...
func sweepTimedOutTasks(ctx context.Context) {
	...
	legacyReason := "task timed out (legacy task from old system, no refund, please contact admin)"
	now := time.Now().Unix()
	timedOutCount := 0
...
func UpdateSunoTasks(ctx context.Context, taskChannelM map[int][]string, taskM map[string]*model.Task) error {
	for channelId, taskIds := range taskChannelM {
		...
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf("channel #%d async task update failed: %s", channelId, err.Error()))
		}
	}
	return nil
}
...
func updateSunoTasks(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	logger.LogInfo(ctx, fmt.Sprintf("channel #%d pending tasks: %d", channelId, len(taskIds)))
	if len(taskIds) == 0 {
		return nil
	}
	...
	if !responseItems.IsSuccess() {
		common.SysLog(fmt.Sprintf("channel #%d pending tasks: %d, response: %s", channelId, len(taskIds), string(responseBody)))
		return err
	}
