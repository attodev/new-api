...
func initCol() {
	...
	if os.Getenv("LOG_SQL_DSN") != "" {
		...
		// when LOG_SQL_DSN is empty, log database is the same as the main database
		if common.UsingPostgreSQL {
			logGroupCol = `"group"`
			logKeyCol = `"key"`
		} else {
...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&OrganizationSubscriptionPreConsumeRecord{}, "OrganizationSubscriptionPreConsumeRecord"},
		{&CustomOAuthProvider{}, "CustomOAuthProvider"},
		{&UserOAuthBinding{}, "UserOAuthBinding"},
		{&PerfMetric{}, "PerfMetric"},
	}
	// dynamically compute migration count to ensure errChan buffer is large enough
...
func checkMySQLChineseSupport(db *gorm.DB) error {
	// check: schema default charset/collation + per-table collations (implied charset)
	...
	isChineseCapable := func(cs, cl string) bool {
		...
		// if only collation is provided, try to determine by collation prefix
	...
	// 1) schema default must support Chinese characters
	...
	// 2) all physical table collations (implied charset) must support Chinese characters
	...
	for _, t := range tables {
		// NULL or empty means inheriting schema default, already validated above — treat as pass
		if t.Collation == nil || *t.Collation == "" {
			continue
		}
		cl := *t.Collation
		// determine Chinese capability from collation alone
		ok := false
	...
	if len(badTables) > 0 {
		// limit output count to avoid overly long log messages
		maxShow := 20
		shown := badTables
