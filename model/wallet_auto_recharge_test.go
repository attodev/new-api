...
func stubWalletCharger(done bool, total int64, err error) TossBillingCharger {
	return func(ctx context.Context, billingKey, customerKey, secretKey, orderID, orderName string, amount int64) (*TossBillingChargeResult, error) {
		...
		return &TossBillingChargeResult{Done: done, Total: total, PaymentKey: "pay_" + orderID}, nil
	}
}
...
func TestActivateMonthlyWalletAutoRechargeIgnoresImmediateCharge(t *testing.T) {
	...
	called := false
	activated, err := ActivateWalletAutoRechargeFromToss("monthly-immediate-auth", 1, "card", "****1234", false, func(ctx context.Context, billingKey, customerKey, secretKey, orderID, orderName string, amount int64) (*TossBillingChargeResult, error) {
		called = true
