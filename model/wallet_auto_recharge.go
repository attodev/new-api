...
func ProcessWalletAutoRecharge(ctx context.Context, policyId int, now time.Time, maxFails int, charger TossBillingCharger) error {
	...
	if !charge.alreadyDone {
		result, err := charger(ctx, charge.billingKey, charge.customerKey, charge.secretKey, charge.tradeNo, "지갑 자동충전", charge.chargeKRW)
		if err != nil || result == nil || !result.Done || result.Total != charge.chargeKRW {
			if err == nil {
...
func prepareWalletAutoRechargeCharge(policyId int, now time.Time, maxFails int) (walletAutoRechargeCharge, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		billingKey, customerKey, secretKey, err := getTossBillingKeyPlainWithSecretTx(tx, policy.BillingKeyId)
		if err != nil {
