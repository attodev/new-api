...
var (
	...
	ErrFriendlyPasskeyNotFound = errors.New("passkey verification failed, please retry or contact admin")
)
...
func GetPasskeyByUserID(userID int) (*PasskeyCredential, error) {
	...
	if err := DB.Where("user_id = ?", userID).First(&credential).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			// not found is normal (user has not bound passkey) — return ErrPasskeyNotFound without logging
...
func UpsertPasskeyCredential(credential *PasskeyCredential) error {
	if credential == nil {
		...
		return fmt.Errorf("passkey save failed, please retry")
	}
	return DB.Transaction(func(tx *gorm.DB) error {
		// use Unscoped() for hard delete to avoid unique index conflicts
...
func DeletePasskeyByUserID(userID int) error {
	if userID == 0 {
		...
		return fmt.Errorf("delete failed, please retry")
	}
	// use Unscoped() for hard delete to avoid unique index conflicts
