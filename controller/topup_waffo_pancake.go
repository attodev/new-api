...
func RequestWaffoPancakeAmount(c *gin.Context) {
	...
	if req.Amount < int64(setting.WaffoPancakeMinTopUp) {
		common.ApiErrorI18n(c, i18n.MsgTopupAmountTooSmall, map[string]any{"Min": setting.WaffoPancakeMinTopUp})
		return
	}
...
func CreateWaffoPancakePair(c *gin.Context) {
	...
	if err != nil {
		...
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Waffo Pancake create store and product failed orphan_store=%t store_id=%q error=%q",
			orphan, func() string {
				if result == nil {
					return ""
				}
...
func RequestWaffoPancakePay(c *gin.Context) {
	if !isWaffoPancakeTopUpEnabled() {
		common.ApiErrorI18n(c, i18n.MsgTopupWaffoPancakeNotConfig)
		return
	}
...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	if expectedEnv != "test" && expectedEnv != "prod" {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf(
			"Waffo Pancake webhook path env segment invalid env=%q path=%q client_ip=%s",
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook read body failed path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), err.Error()))
		c.String(http.StatusBadRequest, "bad request")
		return
	}

	signature := c.GetHeader("X-Waffo-Signature")
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook received path=%q client_ip=%s signature=%q body=%q", c.Request.RequestURI, c.ClientIP(), signature, string(bodyBytes)))
	...
	if isSubscription {
		...
		if err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf(
				"Waffo Pancake webhook subscription order parse failed event_id=%s order_id=%s buyer_identity=%q client_ip=%s error=%q",
	...
	if err != nil {
		...
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Waffo Pancake webhook order parse failed event_id=%s order_id=%s buyer_identity=%q client_ip=%s error=%q",
			event.ID, event.Data.OrderID, event.Data.MerchantProvidedBuyerIdentity, c.ClientIP(), err.Error(),
		))
		c.String(http.StatusOK, "OK")
		return
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake recharge succeeded trade_no=%s event_id=%s order_id=%s client_ip=%s", tradeNo, event.ID, event.Data.OrderID, c.ClientIP()))
	c.String(http.StatusOK, "OK")
}
