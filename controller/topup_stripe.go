...
func (*StripeAdaptor) RequestAmount(c *gin.Context, req *StripePayRequest) {
	if req.Amount < getStripeMinTopup() {
		common.ApiErrorI18n(c, i18n.MsgTopupAmountTooSmall, map[string]any{"Min": getStripeMinTopup()})
		return
	}
	if req.Amount > 10000 {
...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Stripe create checkout session failed user_id=%d trade_no=%s amount=%d error=%q", id, referenceId, req.Amount, err.Error()))
	...
	err = topUp.Insert()
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Stripe create topup order failed user_id=%d trade_no=%s amount=%d error=%q", id, referenceId, req.Amount, err.Error()))
		common.ApiErrorI18n(c, i18n.MsgPaymentCreateFailed)
		return
	}
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Stripe topup order created user_id=%d trade_no=%s amount=%d money=%.2f", id, referenceId, req.Amount, chargedMoney))
...
func StripeWebhook(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("Stripe webhook read body failed path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), err.Error()))
		c.AbortWithStatus(http.StatusServiceUnavailable)
		return
	}

	signature := c.GetHeader("Stripe-Signature")
	logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook received path=%q client_ip=%s signature=%q body=%q", c.Request.RequestURI, c.ClientIP(), signature, string(payload)))
	...
	default:
		logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook event ignored event_type=%s client_ip=%s", string(event.Type), callerIp))
	}
...
func sessionCompleted(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if paymentStatus != "paid" {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe Checkout payment pending, waiting for async result trade_no=%s payment_status=%s client_ip=%s", referenceId, paymentStatus, callerIp))
		return
	}
...
func sessionAsyncPaymentFailed(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if len(referenceId) == 0 {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe async payment failed event missing trade_no client_ip=%s", callerIp))
		return
	}
	...
	if topUp.PaymentProvider != model.PaymentProviderStripe {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe async payment failed but order provider mismatch trade_no=%s payment_provider=%s client_ip=%s", referenceId, topUp.PaymentProvider, callerIp))
		return
	}

	if topUp.Status != common.TopUpStatusPending {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe async payment failed but order not pending, ignoring trade_no=%s status=%s client_ip=%s", referenceId, topUp.Status, callerIp))
		return
	}

	topUp.Status = common.TopUpStatusFailed
	if err := topUp.Update(); err != nil {
		logger.LogError(ctx, fmt.Sprintf("Stripe mark topup order failed status error trade_no=%s client_ip=%s error=%q", referenceId, callerIp, err.Error()))
		return
	}
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) {
	if len(referenceId) == 0 {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe fulfill order missing trade_no client_ip=%s", callerIp))
		return
	}
	...
	if err := model.CompleteSubscriptionOrder(referenceId, common.GetJsonString(payload), model.PaymentProviderStripe, ""); err == nil {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe subscription order completed trade_no=%s event_type=%s client_ip=%s", referenceId, string(event.Type), callerIp))
		return
	...
	currency := strings.ToUpper(event.GetObjectValue("currency"))
	logger.LogInfo(ctx, fmt.Sprintf("Stripe recharge succeeded trade_no=%s amount_total=%.2f currency=%s event_type=%s client_ip=%s", referenceId, total/100, currency, string(event.Type), callerIp))
}
...
func sessionExpired(ctx context.Context, event stripe.Event) {
	...
	if len(referenceId) == 0 {
		logger.LogWarn(ctx, "Stripe checkout.expired missing trade_no")
		return
	}
	...
	if errors.Is(err, model.ErrTopUpNotFound) {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe topup order not found, cannot mark expired trade_no=%s", referenceId))
		return
	}
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("Stripe topup order expiry processing failed trade_no=%s error=%q", referenceId, err.Error()))
		return
	}
