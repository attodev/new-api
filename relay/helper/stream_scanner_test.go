...
//
// Reproduces a real billing bug: the deferred cleanup closed resp.Body only
// after waiting (up to 5s) for goroutines to exit, but the scanner goroutine
// blocks on reading that very resp.Body - so on client disconnect, the
// upstream connection (and its token generation) stayed alive for up to
// those extra seconds instead of being torn down immediately.
