...
func wan27TestRelayInfo() *relaycommon.RelayInfo {
	return &relaycommon.RelayInfo{
		ChannelMeta: &relaycommon.ChannelMeta{},
	}
}

// TestConvertToAliRequestWan27I2VBuildsMediaFromImage reproduces a real
// feature gap: Wan2.7 image-to-video switched to a new input.media protocol
// (a list of {type, url} entries) instead of the legacy img_url/
// first_frame_url/last_frame_url fields, but convertToAliRequest only ever
// populated ImgURL from req.InputReference - a wan2.7-i2v request built from
// the more common "image" field would reach upstream with no media at all.
func TestConvertToAliRequestWan27I2VBuildsMediaFromImage(t *testing.T) {
	...
	aliReq, err := adaptor.convertToAliRequest(wan27TestRelayInfo(), req)
...
func TestConvertToAliRequestWan27I2VRequiresMedia(t *testing.T) {
	...
	_, err := adaptor.convertToAliRequest(wan27TestRelayInfo(), req)

	require.Error(t, err)
	require.True(t, strings.Contains(err.Error(), "requires image"))
}

// TestConvertToAliRequestWan25I2VKeepsLegacyImgURL guards against a
// regression in the other direction: older Wan models (wan2.5 and earlier)
// must keep using the legacy img_url field, not the new media protocol.
