...
// TestOllamaChatHandlerNonStreamToolCalls reproduces a real bug: the
// non-streaming Ollama handler never read message.tool_calls at all (only
// the streaming path did), silently dropping any tool call the model made
// when the client requested a non-streaming response.
