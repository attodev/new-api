...
// TestDispatchAdaptorInit reproduces a real feature gap: Tencent added a
// TokenHub API key format (a single OpenAI-compatible key) alongside its
// legacy three-segment "appid|secretId|secretKey" TC3-signed key, but the
// channel only ever used the native TC3 Adaptor - a TokenHub key would be
// sent through TC3 signing and rejected upstream instead of being routed
// through the OpenAI-compatible protocol TokenHub actually speaks.
