...
// normalizeOpenAIUsage maps the OpenAI Images API's input_tokens/
// output_tokens/input_tokens_details shape onto the common
// prompt_tokens/completion_tokens/prompt_tokens_details fields used for
// billing. This handler is only used on the OpenAI/xAI Images (generations/
// edits) relay path, which never reports prompt_tokens_details directly -
// so cached/cache-write/audio detail must come from here too, or cached
// image-input tokens get silently billed as uncached.
...
// OpenaiImageStreamHandler relays a streaming OpenAI Images API response
// (stream=true: partial_image/completed SSE events) to the client, tracking
// usage from the final data frame. If the upstream ignores stream=true and
// returns a plain JSON body instead, it falls back to wrapping that JSON as
// a synthetic single-event stream so the client still gets the SSE shape it
// asked for.
...
// OpenaiImageJSONAsStreamHandler wraps a plain JSON ImageResponse as a
// synthetic single-event SSE stream, for upstreams that ignore stream=true
// on the Images API and answer with the ordinary JSON shape anyway.
