...
// TestConvertImageEditRequestKeepsValidMultipartStreamFields reproduces a
// real bug: ConvertImageRequest rebuilt the outbound multipart body from
// c.Request.MultipartForm, but if that cache was nil it fell back to
// c.MultipartForm() - reading straight off c.Request.Body - instead of the
// buffered body storage other retry-safe paths use. On a channel retry the
// second attempt could get an empty/already-drained body.
...
// TestConvertImageEditRequestParsesReusableMultipartWhenFormIsMissing
// reproduces the retry scenario directly: c.Request.MultipartForm is nil
// (as it would be on a fresh attempt against a different channel) and only
// the buffered body storage holds the original multipart body.
