...
조직 관리자는 조직 내 사용자 운영과 사용량 모니터링을 담당한다. 사용자 quota, 상태, remark를 관리하고 조직 대시보드와 로그를 볼 수 있다. 조직 지갑 충전은 조직 소유자가 수행한다.
...
조직 소유자로 로그인한 뒤 사이드바에서 `조직 사용자`로 이동한다.

### 2. 사용자 할당 버튼 클릭

할당 가능한 사용자 목록에서 조직에 추가할 사용자를 선택한다.
...
사용자가 조직에 추가된다. 추가된 사용자는 조직 메뉴 또는 조직 사용자용 지갑 화면을 볼 수 있다.

## 조직 소유자/관리자: 조직 사용자 관리하기
